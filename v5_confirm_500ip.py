...
from bond_dkl_v5 import (
    ...
    train_one_epoch, evaluate, save_checkpoint, reinitialize_inducing_points_kmeans, predict
)
from plotting import plot_predictions_vs_actual, plot_residuals, plot_uncertainty_vs_actual, plot_calibration
...
# CONFIRMATION RUN: v5's winning architecture ([128,64,32], gelu)
CONFIGS = [
    {'run_id': 'confirm_ld8', 'latent_dim': 8},
    {'run_id': 'confirm_ld16', 'latent_dim': 16},
]
...
RESULTS_LOG = "v5_confirm_500ip_results.csv"
LOSS_LOG = "v5_confirm_500ip_losses.csv"
CHECKPOINT_DIR = "v5_confirm_500ip_checkpoints"
os.makedirs(CHECKPOINT_DIR, exist_ok=True)

if not os.path.exists(RESULTS_LOG):
    with open(RESULTS_LOG, 'w', newline='') as f:
        csv.writer(f).writerow(['run_id', 'latent_dim', 'num_inducing',
...
for cfg in CONFIGS:
    ...
    print(f"\n=== {run_id}: latent_dim={latent_dim}, inducing={NUM_INDUCING} ===")
    ...
    # Diagnostic plots for THIS config
    test_loader_for_plot = fast_in_memory_batches(X_test_full, y_test_full, batch_size=BATCH_SIZE, shuffle=False)
    preds, actuals, variance = predict(model, likelihood, test_loader_for_plot, device=device, return_variance=True)
    preds_real, actuals_real = np.expm1(preds), np.expm1(actuals)

    plot_predictions_vs_actual(preds_real, actuals_real, title=f"{run_id} (500 inducing): Predicted vs Actual")
    plot_residuals(preds_real, actuals_real, title=f"{run_id} (500 inducing): Residuals")

    # does the GP's own uncertainty widen in the sparse, high-spread
    # tail — the same region where the point prediction ceiling showed up
    # at 100 inducing points?
    plot_uncertainty_vs_actual(preds, actuals, variance, title=f"{run_id} (500 inducing): GP Uncertainty vs Actual")
    plot_calibration(preds, actuals, variance, title=f"{run_id} (500 inducing): Calibration")

print("\nConfirmation run complete. Results in", RESULTS_LOG)
print("Compare against v5_20/v5_17 (same architecture at 100 inducing) —")
print("does the >1.0 prediction ceiling disappear at 500 inducing points?")
