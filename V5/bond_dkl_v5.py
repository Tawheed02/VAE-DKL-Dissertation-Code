...
class FeatureExtractor(torch.nn.Module):
    def __init__(self, input_dim=6, latent_dim=1, hidden_dims=None, activation="silu"):
        """
        v5: activation function configurable (silu/relu/gelu/tanh),
...
def build_optimizer(model, likelihood, nn_lr=0.001, gp_lr=0.01):
    """
    v4: split learning rate — the feature extractor (non-convex landscape,
    needs smaller careful steps) gets nn_lr; GP-related parameters (kernel,
    mean, variational strategy, likelihood — comparatively well-behaved)
    get the faster gp_lr. (L2 weight decay removed — not part of the
    confirmed model decisions being tested, keeping this grid scoped.)
    """
...
def train_one_epoch(model, likelihood, optimizer, mll, train_loader, device='cpu',
    ...
    mixed precision (autocast) — NOT the GP/kernel computation, which stays
    in full FP32.
    """
    model.train()
    likelihood.train()
...
def reinitialize_inducing_points_kmeans(model, sample_loader, latent_dim, num_inducing_points, device='cpu'):
    """
    v4: after some warmup epochs with random inducing points,
    runs k-means on the resulting Z values, and replaces the
    inducing point locations with the cluster centers
    """
    model.eval()
    z_samples = []
    with torch.no_grad():
...
def predict(model, likelihood, loader, device='cpu'):
    model.eval()
    likelihood.eval()
